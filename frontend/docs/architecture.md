...
`/projects/:projectId?tab=recommendations` is available to the same organization administrator and supervisor roles as project details. It uses `GET /projects/:projectId/recommendations` with `['recommendations', organizationId, projectId]` for the latest saved recommendation run, plus `POST /projects/:projectId/recommendations` to synchronously create a new saved run. Generation requires the backend’s latest project document analysis and at least one organization employee. The backend provides rank-ordered results, and the client preserves that order without sorting or recalculating scores. It displays returned score, reasons, skill alignment, workload, availability, performance, capacity, suitability, and optional score-breakdown values. Recommendations remain advisory; no assignment action is presented. There is no snapshot history, deletion, or regeneration endpoint distinct from generating a new latest run.
