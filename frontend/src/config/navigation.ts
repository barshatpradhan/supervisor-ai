...
export function getNavigationItems(input: {
  ...
  const permittedItems = navigation.filter((item) => !item.permission || hasPermission({
    permission: item.permission,
    organizationRole: input.organizationRole,
    platformRole: input.platformRole,
  }))

  if (input.organizationRole === 'organization_admin') {
    return [...permittedItems, { href: '/team', label: 'Team' }]
  }

  if (input.organizationRole === 'supervisor') {
    return [...permittedItems, { href: '/employees', label: 'Employees' }]
  }
